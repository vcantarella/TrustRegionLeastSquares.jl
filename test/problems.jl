...
powell_singular(x) =
    [x[1] + 10x[2], sqrt(5) * (x[3] - x[4]), (x[2] - 2x[3])^2, sqrt(10) * (x[1] - x[4])^2]
kowalik_osborne(x) =
...
function watson(x)
    n = length(x)
    r = [
        sum((j - 1) * x[j] * t^(j - 2) for j = 2:n) - sum(x[j] * t^(j - 1) for j = 1:n)^2 - 1 for t in (1:29) ./ 29
    ]
...
const MGH = [
    ...
    (name = "Powell badly scaled", r = powell_badly_scaled, x0 = [0.0, 1.0], Fstar = 0.0),
