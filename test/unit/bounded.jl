...
@testset "linear residual x − target: optimum is the projection of target onto the box" begin
    ...
    for (target, lb, ub) in (
        ...
        x, f, g, iter =
            NL.lm_trust_region!(res!, jac!, x0, 3, strategy; lb, ub, gtol = 1e-10)
...
@testset "bound handling: fixed variable, infeasible start, one-sided, interior" begin
    ...
    x, f, g, iter =
        NL.lm_trust_region!(res!, jac!, [5.0, 5.0], 2; ub = [0.5, 1.0], gtol = 1e-8)
    @test x ≈ [0.5, 0.25] atol = 1e-6
    @test x[1] == 0.5
    ...
    x, f, g, iter =
        NL.lm_trust_region!(res!, jac!, [1.0, 1.0], 3; ub = [2.0, Inf], gtol = 1e-8)
    @test x[1] == 2.0 && kkt(x, g, [-Inf, -Inf], [2.0, Inf])
    x, f, g, iter =
        NL.lm_trust_region!(res!, jac!, [1.0, 1.0], 3; lb = [-Inf, 0.6], gtol = 1e-8)
    @test x[2] == 0.6 && kkt(x, g, [-Inf, 0.6], [Inf, Inf])
    @test_throws ArgumentError NL.lm_trust_region!(
        res!,
        jac!,
        [1.0, 1.0],
        3;
...
@testset "Coleman–Li distances" begin
    ...
    @test NL.coleman_li_distances!(zeros(4), x, g, lb, ub) == [0.5, 0.3, 1.0, 1.0]   # ub − x, x − lb, ∞ bound, ∞ bound
    @test NL.coleman_li_distances!(zeros(4), [1.0, 0.2, 0.5, 0.5], g, lb, ub)[1:2] ==
...
@testset "projected gradient norm" begin
    ...
    @test NL.projected_gradient_norm(g, x, fill(-Inf, 6), fill(Inf, 6)) ≈ norm(g) rtol =
        1e-15   # |x| ≫ |g|: no cancellation
    @test NL.projected_gradient_norm([-3.0, 2.0], [1.0, 0.0], [-Inf, 0.0], [1.0, Inf]) == 0        # on bounds, gradient outwards
    @test NL.projected_gradient_norm([-3.0, 2.0], [0.9, 0.5], [-Inf, 0.0], [1.0, Inf]) ≈
          sqrt(0.1^2 + 0.5^2)
end
...
@testset "Cauchy step: ω is limited by the model, the trust region, or the box" begin
    ...
    x = [0.9, 0.5, 0.5, 0.5]
    cache = NL.BoundedCache(NL.subproblem_cache_init(NL.QRStrategy(), NL.NoScaling(), J))
    ...
    function candidate_ω(x, g, Δ, lb, ub)
        D = NL.affine_scaling!(cache, x, g, lb, ub)
        d = -NL.coleman_li_distances!(zeros(m), x, g, lb, ub) .* g
        box = minimum(
    ...
    @testset "pC = ω d with ω the smallest of the three candidates: $name" for (
        ...
        decrease = NL.cauchy_step!(cache, J, g, x, Δ, lb, ub)
    ...
    @testset "a KKT point gives a zero step" begin
        ...
        NL.affine_scaling!(cache, x_kkt, g, lb, ub)
        @test NL.cauchy_step!(cache, J, g, x_kkt, 1.0, lb, ub) == 0
    end
end
...
@testset "Cauchy-fraction safeguard" begin
    ...
    g = J' * f
    cache = NL.BoundedCache(NL.subproblem_cache_init(NL.QRStrategy(), NL.NoScaling(), J))
    NL.affine_scaling!(cache, x, g, lb, ub)
    model_decrease(p) = -dot(g, p) - sum(abs2, J * p) / 2
    decrease_cauchy = NL.cauchy_step!(cache, J, g, x, 1e3, lb, ub)

    # An ascent step must be pulled towards pC until the fraction β₁ holds with equality.
    cache.inner.p .= 0.1 * g
    predicted = NL.projected_step!(cache, J, g, x, lb, ub, decrease_cauchy; β₁ = 0.1)
